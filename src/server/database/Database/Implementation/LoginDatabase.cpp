...
#define BnetGameAccountInfo "a.id, a.username, ab.unbandate, ab.unbandate = ab.bandate, aa.SecurityLevel"
    ...
    PrepareStatement(LOGIN_INS_HWID_BAN, "INSERT INTO hwid_bans (accountID, physicalDriveID, cpuID, volumeInformation, ipAddress, banned, isVM) VALUES (?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(LOGIN_UPD_HWID_BAN_ACC, "UPDATE hwid_bans SET banned = ? WHERE accountID = ?", CONNECTION_ASYNC);
    PrepareStatement(LOGIN_UPD_HWID_INFO_IP, "UPDATE hwid_bans SET ipAddress = ?, isVM = ? WHERE accountID = ?", CONNECTION_ASYNC);
