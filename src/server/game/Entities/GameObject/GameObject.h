...
class TC_GAME_API GameObject : public WorldObject, public GridObject<GameObject>, public MapObject
{
    ...
    private:
        bool Create(uint32 entry, Map* map, Position const& pos, QuaternionData const& rotation, uint32 animProgress, GOState goState, uint32 artKit, bool hasDoodads,  float visibility = SIZE_OF_GRIDS, float size = -1.0f, bool dynamic, ObjectGuid::LowType spawnid);
