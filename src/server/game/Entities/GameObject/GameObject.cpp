...
bool GameObject::Create(uint32 entry, Map* map, Position const& pos, QuaternionData const& rotation, uint32 animProgress, GOState goState, uint32 artKit, bool dynamic, ObjectGuid::LowType spawnid, bool hasDoodads, float visibility /* 533.333f */, float size /*= -1*/)
...
bool GameObject::LoadFromDB(ObjectGuid::LowType spawnId, Map* map, bool addToMap, bool)
{
    ...
    if (!Create(entry, map, data->spawnPoint, data->rotation, animprogress, go_state, artKit, hasDoodads, visibility, size, !m_respawnCompatibilityMode, spawnId))
        return false;
